...
<p align="center">
  <img src="web/public/icons/workspace-mark.svg" alt="澜序标志" width="96" />
</p>
...
<p align="center">
  <a href="https://github.com/uhhgxg/lanxu">GitHub</a> ·
  <a href="docs/API.md">API</a> ·
  <a href="docs/ACL-MATRIX.md">权限模型</a> ·
  <a href="SECURITY.md">安全策略</a>
</p>

由 [uhhgxg](https://github.com/uhhgxg) 开发与维护。源码、问题反馈及后续更新统一在本仓库进行；许可条款见 [LICENSE](LICENSE)。

> 一句话：聊天界面让你向 Agent 提问；澜序让 Agent 拥有长期运行的工作区、可审计的能力边界，以及能够被 Web、桌面端和消息渠道共同使用的运行时。

产品采用智能体优先工作模型：先明确 Agent 的身份、能力和运行边界，再把 Workspace、Memory、工具与渠道接入同一条可追踪的执行链。

## 产品定位

澜序是一个面向个人与团队的自托管 AI Agent 工作台。它不是一个只保存聊天记录的对话框，而是把以下对象放在同一个产品模型中：
...
澜序采用本地优先和显式集成的思路：数据库、工作区元数据、会话状态和配置由自己的服务管理；模型、消息渠道、Docker 与外部工具都作为可配置边界接入。
...
<p align="center">
  <img src="docs/screenshots/workbench.png" alt="澜序智能体工作台" width="900" />
</p>
...
<p align="center">
  <img src="docs/screenshots/capabilities.png" alt="澜序能力库" width="49%" />
  <img src="docs/screenshots/settings-models.png" alt="澜序模型配置" width="49%" />
</p>
...
澜序图标同时用于 Web/PWA、Electron 窗口和安装包资源：

<p align="center">
  <img src="web/public/icons/workspace-mark.svg" alt="澜序应用标志" width="160" />
</p>
...
- 复用内置的 MCP capability handler，并映射为稳定的 `mcp__miniclaw__*` 工具。
...
- 同一套 Renderer 可以连接本地 Backend，也可以通过 `MINICLAW_SERVER_URL` 连接远程澜序服务。
...
```bash
git clone https://github.com/uhhgxg/lanxu.git
cd lanxu
...
Agent 容器镜像默认使用 `uhhgxg/lanxu-agent:latest`，可以通过 `MINICLAW_CONTAINER_IMAGE` 或 `CONTAINER_IMAGE` 覆盖。当前仓库尚未自动发布镜像；使用容器模式前，请先按 `container/Dockerfile` 自行构建并使用该标签，或将变量设为已发布的镜像地址。
...
| `MINICLAW_CONTAINER_IMAGE` | Agent Runner 使用的容器镜像 | `uhhgxg/lanxu-agent:latest` |
...
- [`docs/miniclaw-migration-cleanup.md`](docs/miniclaw-migration-cleanup.md) — 内部兼容命名记录
...
澜序使用 MIT License，详见 [`LICENSE`](LICENSE)。
