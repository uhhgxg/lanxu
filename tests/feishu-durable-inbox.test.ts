...
const { scopeChannelJid } = await import('../src/channel-address.js');
...
describe('Feishu durable Inbox and cursor integration', () => {
  test('registers the first account-scoped P2P DM before strict route resolution', async () => {
    const accountId = `account-first-p2p-${Date.now()}`;
    ...
    const resolveEffectiveChatJid = vi.fn((jid: string) => {
      if (!registered) {
        throw new Error(`strict route rejected before registration: ${jid}`);
      }
      return {
        effectiveJid: 'web:durable-feishu-test',
        agentId: null,
    ...
    const connected = await connect(accountId, executed, {
      normalizeIncomingJid: (jid) => scopeChannelJid(jid, accountId),
      onNewChat,
      resolveEffectiveChatJid,
    });

    await connected.handler(
      event('om_first_account_scoped_p2p', Date.now(), 'first dm'),
    );

    const expectedJid = scopeChannelJid('feishu:ou_durable_user', accountId);
    expect(onNewChat).toHaveBeenCalledWith(expectedJid, '飞书私聊');
    expect(resolveEffectiveChatJid).toHaveBeenCalledWith(
      expectedJid,
      expect.objectContaining({ chatType: 'p2p' }),
    );
    expect(onNewChat.mock.invocationCallOrder[0]).toBeLessThan(
      resolveEffectiveChatJid.mock.invocationCallOrder[0],
    );
    expect(executed).toHaveBeenCalledWith('om_first_account_scoped_p2p');
  });
