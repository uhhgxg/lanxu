...
describe('AgentProfile DB model', () => {
  test('creates one default AgentProfile for every new user', () => {
    ...
    expect(profiles[0].name).toBe('澜序');
    ...
    expect(profiles[0].identity_hash).toBe(
      computeAgentProfileIdentityHash('', true, undefined, '澜序'),
    );
  });

  test('persists admin 澜序 as managed; global host policy is resolved at runtime', () => {
  ...
  test('migrates the former Miniclaw default name to 澜序', () => {
  ...
  test('permanently binds Home Workspace to the built-in 澜序 Agent', () => {
