...
bugReportRoutes.post('/submit', authMiddleware, async (c) => {
  ...
  const fullBody = `${body}\n\n---\n> Submitted via 澜序 by ${user.display_name || user.username}`;
  ...
  const url = `https://github.com/uhhgxg/lanxu/issues/new?title=${encodeURIComponent(title)}&body=${encodeURIComponent(truncatedBody)}`;
