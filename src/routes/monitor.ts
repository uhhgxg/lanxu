...
monitorRoutes.get('/status', authMiddleware, async (c) => {
  ...
  let providerPoolSummary:
    | {
        strategy: ReturnType<typeof getBalancingConfig>['strategy'];
  ...
  if (isAdmin) {
    ...
    const statuses = providerPool.getHealthStatuses().map((status) => ({
      ...status,
      name: providerNameMap.get(status.profileId) ?? status.profileId,
    }));
    providerPoolSummary = {
      strategy: balancing.strategy,
      unhealthyThreshold: balancing.unhealthyThreshold,
      recoveryIntervalMs: balancing.recoveryIntervalMs,
      enabledCount: enabledProviders.length,
      healthyCount: statuses.filter((status) => status.healthy).length,
      statuses,
    };
  }
  ...
  return c.json({
    ...
    agentOperations: {
      ...
      providerPool: providerPoolSummary,
      deliveryReview: isAdmin
        ? (() => {
            const uncertain = listUncertainChannelOutbox(101);
            return {
              count: Math.min(uncertain.length, 100),
              capped: uncertain.length > 100,
            };
          })()
        : undefined,
    },
