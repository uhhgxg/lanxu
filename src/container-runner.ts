...
export interface ContainerInput {
  ...
  agentProfile?: {
    ...
    name: string;
    version: number;
    /** Host-authoritative marker for the built-in 澜序 AgentProfile. */
    isDefault: boolean;
    identityHash: string;
  ...
  /**
   * True only for an interactive turn in the built-in 澜序 Home
   ...
   * actual-owner interactive turn of built-in 澜序 in Home. The runner
  ...
  /** Host-derived capability flag for main-澜序 interactive sessions. */
...
export function buildVolumeMounts(
  ...
  ensureSettingsJson(
    ...
    resolveRuntimeMcpServers(group, agentProfile),
    {
      // The session settings file is 澜序-owned. Always replace this map
  ...
  // authorization, forcing re-auth every IDLE_TIMEOUT (#477). 澜序 never
...
export function detectContainerHostIdentity(
  ...
  // while 澜序 is running; reusing an earlier direct result could bypass
...
export async function runHostAgent(
  ...
  // from the 澜序 parent process leak into a host-mode child after the
