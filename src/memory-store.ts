...
/** One durable Home Workspace fact completes 澜序's first-wake ritual. */
...
function rejectReservedCanonicalKey(value: string | null | undefined): void {
  ...
  throw new WorkspaceMemoryStoreError(
    'reserved_canonical_key',
    'This canonical key is owned by the 澜序 Owner Profile service',
  );
}
...
function rejectReservedMemoryItem(item: WorkspaceMemoryItem): void {
  ...
  throw new WorkspaceMemoryStoreError(
    'reserved_canonical_key',
    'This memory item is owned by the 澜序 Owner Profile service',
  );
}
 ...
 *
 * 澜序 may temporarily disable SQLite foreign-key enforcement when a
  ...
  return db.transaction(() => {
    ...
    if (!current) {
      if (input.clear) {
        throw new WorkspaceMemoryStoreError(
          'revision_conflict',
          '澜序 Owner Profile revision conflict',
