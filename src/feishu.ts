...
export interface FeishuConnectionConfig {
  appId: string;
  appSecret: string;
  /** Optional 澜序 account id. The scoped source JID remains authoritative. */
  ...
  async function processClaimedIncomingMessage(
    ...
    try {
      ...
      if (chatType === 'p2p' && resolveEffectiveChatJid) {
        // Account-scoped resolvers fail closed by throwing when a route does
        // not exist; they do not return null.  Probing the resolver before
        // registration therefore prevented the first DM from ever reaching
        // onNewChat.  These callbacks are intentionally idempotent, so run the
        // P2P bootstrap first and let the authoritative admission check below
        // validate the newly registered route exactly once.
      ...
      try {
        ...
        while (hasMore) {
          ...
          for (const chat of items) {
            ...
            // missed or the chat has never sent a message to 澜序. The
