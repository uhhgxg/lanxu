...
export function feedStreamEventToCard(
  ...
  switch (se.eventType) {
    ...
    case 'prompt_suggestion':
      ...
      break;
    case 'context_audit':
      // Context audits describe 澜序/Claude runtime wiring. Keep them in
...
/** A paired/registered direct chat is already bound to one 澜序 user.
  ...
  if (isRecovery) {
    ...
    historyContext = buildRecentConversationHistoryContext(
      ...
      new Set(missedMessages.map((m) => m.id)),
      {
        ...
        intro:
          '检测到当前 workspace 切换或更新了顶层 AgentProfile 身份提示词，底层模型 session 已重置。以下是 澜序 保存的最近对话记录，供你在新身份下延续上下文。',
      },
    );
    if (historyContext) {
      logger.info(
    ...
    historyContext = buildRecentConversationHistoryContext(
      ...
      new Set(missedMessages.map((m) => m.id)),
      {
        ...
        maxMessageLength: 700,
        intro:
          '检测到本次因切换 provider 需要使用新的底层模型 session。以下是 澜序 保存的最近对话记录，供你延续上下文。',
      },
    );
    if (historyContext) {
      logger.info(
...
async function processTaskIpc(
  data: {
    ...
    // Dedicated built-in 澜序 Owner Profile facade.
    preferredAddress?: string;
    expectedOnboardingRevision?: number;
    leaseToken?: number;
    // For the main 澜序's conversational Agent Builder
    profileId?: string;
    draftId?: string;
    ...
    case 'miniclaw_owner_profile': {
      ...
      try {
        ...
        logger.warn(
          ...
          '澜序 Owner Profile IPC request failed',
        );
      }
      break;
    }
...
async function processAgentConversation(
  ...
  // session was cleared by provider/model switching, inject persisted 澜序
    ...
    historyContext = buildRecentConversationHistoryContext(
      ...
      new Set(missedMessages.map((m) => m.id)),
      {
        ...
        intro: resetForAgentProfile
          ? '检测到当前 workspace 切换或更新了顶层 AgentProfile 身份提示词，当前 agent 的底层模型 session 已重置。以下是 澜序 保存的最近对话记录，供你在新身份下延续上下文。'
          : '检测到当前 agent 的底层模型 session 是新的（可能因为切换 provider/model 或恢复失败）。以下是 澜序 保存的最近对话记录，供你延续上下文。',
      },
    );
    if (historyContext) {
      logger.info(
    ...
    try {
      ...
      if (messages.length > 0) {
        ...
        for (const [chatJid, groupMessages] of messagesByGroup) {
          ...
          // the notify_channels broadcast (historical issue #559).
 ...
 * by a different 澜序 user), re-routing is skipped — the chat stays with
...
function buildOnNewChat(
  ...
  return (chatJid, chatName) => {
    ...
    if (existing) {
      ...
      //      a different 澜序 user → do NOT re-route.
      //
      ...
      if (!existing.is_home) {
        ...
        if (previousOwnerStillConnected) {
          ...
          Object.assign(existing, releaseOwner(existing), {
            ...
            // Trust belongs to the previous 澜序 user's channel binding,
