 ...
 * layer. Skills stay out of this list because 澜序 governs them with an
...
export interface ClaudeContextPlanArgs {
   ...
   * context. Managed 澜序 skills are resolved independently below.
   */
 ...
 * order. The caller applies 澜序's required env and effective MCP policy
...
export function loadHostClaudeSettings(
  ...
  for (const settingsFile of plan.settingsSourceFiles) {
    ...
    try {
      ...
      // directly. 澜序 skips the invalid layer to keep startup usable.
    }
  }
  return merged;
}
...
export function syncHostClaudeContext(
  ...
  // These paths are 澜序-owned projections of the selected native user
