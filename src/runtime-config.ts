...
/**
 * Remove provider-owned values inherited from the 澜序 parent process.
...
function migrateLegacyBrandName(value: unknown, fallback: string): string {
  if (typeof value !== 'string' || !value.trim()) return fallback;
  return value === 'Miniclaw' ? fallback : value;
}
...
export function getAppearanceConfig(): AppearanceConfig {
  try {
    ...
    return {
      appName: migrateLegacyBrandName(
        raw.appName,
        DEFAULT_APPEARANCE_CONFIG.appName,
      ),
      aiName: migrateLegacyBrandName(
        raw.aiName,
