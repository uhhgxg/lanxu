 ...
 * even when CLAUDE_CONFIG_DIR points at Miniclaw's per-session directory.
...
export function resolveManagedHostClaudeMdExcludes(options: {
  ...
  // Miniclaw git repository. Claude Code therefore discovers the repository's
  // own CLAUDE.md as Project memory. That file is useful while developing
  // Miniclaw, but it must not redefine a managed business Agent as a codebase
