...
function resolveDesktopUrls(): { serverUrl: string; rendererUrl: string } {
  const serverValue =
    ...
    DEFAULT_SERVER_URL;
  const serverUrl = normalizeHttpUrl(serverValue, '澜序 server URL');
  const rendererValue =
    getCliValue('--renderer-url') ||
    process.env.MINICLAW_RENDERER_URL ||
    serverUrl;
  const rendererUrl = normalizeHttpUrl(rendererValue, '澜序 renderer URL');
...
function errorPageHtml(failedUrl: string): string {
  ...
  return `<!doctype html>
<html><head><meta charset="utf-8"><title>澜序 is unavailable</title>
...
</head><body><main><h1>澜序 Backend 未连接</h1><p>请先启动 澜序 Backend，或检查桌面应用配置的服务地址：</p><p><code>${safeUrl}</code></p><button id="retry">重试连接</button><script>document.getElementById('retry').addEventListener('click',()=>window.miniclawDesktop?.retry())</script></main></body></html>`;
}
...
function createApplicationMenu(): void {
  ...
  const template: MenuItemConstructorOptions[] = [
    ...(process.platform === 'darwin' ? [{ role: 'appMenu' as const }] : []),
    {
      label: '澜序',
      submenu: [
        { label: 'About 澜序', click: () => void showAbout() },
        { type: 'separator' },
        { role: 'quit' },
      ],
    },
      ...
      submenu: [
        {
          label: '项目源码',
          click: () =>
            void openExternalUrl('https://github.com/uhhgxg/lanxu'),
        },
      ],
    },
  ];
