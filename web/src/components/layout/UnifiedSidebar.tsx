...
export function UnifiedSidebar({
  ...
  const panelWidth = showWorkspaceList ? '18rem' : '0';
    ...
    <TooltipProvider delayDuration={200}>
      <div className="h-full flex flex-shrink-0">
        <nav
          ...
          className="w-[4.5rem] h-full bg-muted/30 flex flex-col items-center py-3 gap-1 flex-shrink-0"
        >
          <div
            data-sidebar-brand="true"
            className="w-11 h-11 rounded-xl overflow-hidden mb-3 flex-shrink-0"
          >
            <img
          ...
          {navItems.map(({ path, icon: Icon, label }) => {
            ...
            const baseClass =
              'nav-item w-12 h-12 rounded-xl flex flex-col items-center justify-center gap-0.5 transition-colors';
            const activeClass = isActive
              ? 'nav-item-active bg-brand-50 text-primary'
        ...
        >
          <div
            ...
            className="w-[18rem] h-full flex flex-col bg-muted/30"
          >
            <div
              data-sidebar-brand="true"
              className="flex items-center gap-2 px-4 pt-6 pb-3 mb-3 flex-shrink-0"
            >
              <div className="min-w-0">
                <img
                  ...
                  className="h-8"
                />
                <p className="mt-1 truncate text-[10px] tracking-[0.04em] text-muted-foreground">
                  私有智能工作台
                </p>
              </div>
              <div className="flex-1" />
      ...
      <ConfirmDialog
        ...
        title="重建工作区"
        message={`确认重建「${clearState.name}」？会永久删除全部聊天记录、上下文、所有子对话及其消息、工作目录文件，以及该工作区的全部 Memory（含版本历史、遗忘记录和 澜序 称呼偏好）；Home 还会重置首次唤醒状态。关联定时任务会停止并移入回收站，运行历史与持久化目录 (data/extra/) 保留。不可撤销。`}
        confirmText="确认重建"
