...
export function ClaudeProviderSection({
  ...
  const handleBalancingSave = useCallback(async () => {
    if (!balancing) return;
    ...
    try {
      ...
      setError(getErrorMessage(err, '保存负载均衡设置失败'));
    } finally {
      setBalancingSaving(false);
    }
  }, [balancing, setError, setNotice]);
    ...
    <div className="space-y-4">
      ...
      {balancing && (
        <BalancingSettings
          ...
          onSave={handleBalancingSave}
          saving={balancingSaving}
          disabled={busy || balancingSaving}
        />
      )}
