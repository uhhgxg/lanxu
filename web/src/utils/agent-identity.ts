 ...
 * the global 澜序 avatar when it owns at least one avatar field.
 */
export function resolveAgentDisplayIdentity({
  ...
  return {
    ...
    imageUrl:
      imageUrl ||
      (!emoji ? `${import.meta.env.BASE_URL}icons/workspace-mark.svg` : undefined),
    emoji,
    color,
